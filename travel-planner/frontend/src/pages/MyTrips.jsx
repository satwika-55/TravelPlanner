...
import { MapPin, Calendar, Trash2, Copy, RefreshCw, Share2, Compass, ArrowLeft, Loader } from "lucide-react";
...
const MyTrips = () => {
  ...
  const fetchUserTrips = async () => {
    try {
      ...
      const userTrips = await getUserTrips(user.email);
      setTrips(userTrips || []);
    } catch (error) {
  ...
  const getImageUrl = (location) => {
    ...
    if (loc.includes("paris")) return "https://images.unsplash.com/photo-1502602898657-3e91760cbb34?auto=format&fit=crop&w=600&q=80";
    if (loc.includes("finland") || loc.includes("lapland")) return "https://images.unsplash.com/photo-1517411032315-54ef2cb783bb?auto=format&fit=crop&w=600&q=80";
    if (loc.includes("kyoto") || loc.includes("japan")) return "https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?auto=format&fit=crop&w=600&q=80";
    if (loc.includes("swiss") || loc.includes("switzerland")) return "https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?auto=format&fit=crop&w=600&q=80";
    if (loc.includes("dubai")) return "https://images.unsplash.com/photo-1512453979798-5ea266f8880c?auto=format&fit=crop&w=600&q=80";
    if (loc.includes("bali")) return "https://images.unsplash.com/photo-1537996194471-e657df975ab4?auto=format&fit=crop&w=600&q=80";
    if (loc.includes("delhi") || loc.includes("india")) return "https://images.unsplash.com/photo-1587474260584-136574528ed5?auto=format&fit=crop&w=600&q=80";
    if (loc.includes("rio") || loc.includes("brazil")) return "https://images.unsplash.com/photo-1483729558449-99ef09a8c325?auto=format&fit=crop&w=600&q=80";
            ...
            {trips.map((trip) => (
              ...
              >
                <div className="h-52 relative overflow-hidden">
                  <img src={getImageUrl(trip.userselection?.location)} alt={trip.userselection?.location} className="w-full h-full object-cover" />
